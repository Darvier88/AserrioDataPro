 ...
 */
public class AñadirDetalleController implements Initializable {

    @FXML
    private VBox mainContainer;
    @FXML
    private GridPane formGrid;
    private AñadirBase<Detalle> añadirBase;
    ...
    public void initialize(URL url, ResourceBundle rb) {
       // Inicializar AñadirBase para la clase Cliente
        añadirBase = new AñadirBase<>(Detalle.class);
        
        // Agregar el formulario generado al VBox principal
        mainContainer.getChildren().add(añadirBase.getFormGrid());
    }    
    private boolean insertarDetalleEnBD(Detalle detalle) {
        String sql = "{call InsertDetalle(?, ?, ?, ?)}"; // Llamada al procedimiento almacenado
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            cstmt.setString(4, detalle.getDetalleAdic());
    ...
    private void añadir(ActionEvent event) {
        ...
        Detalle nuevoDetalle = añadirBase.getObject();

        // Insertar el cliente en la base de datos
        if (insertarDetalleEnBD(nuevoDetalle)) {
            // Limpiar los campos después de añadir
            añadirBase.clearFields();
        } else {
            mostrarError("No se pudo añadir el detalle a la base de datos.");
        }
        App.setRoot("detalle");
            ...
            e.printStackTrace();
            mostrarError("Ocurrió un error al intentar añadir el detalle.");
        }
    }
